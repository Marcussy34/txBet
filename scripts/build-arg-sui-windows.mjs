...
// ---------- per-agent momentum entries (rules fixed ex-ante; trailing data only) ----------
// Every agent reads its own slice of live match state and positions on the
// tournament-path (ADVANCE) book while it still prices the pre-event state:
//   goal-reaction  -> scoring team's advance, entry cap $0.90
//   red-card       -> opposing team's advance, entry cap $0.90
//   penalty-var    -> beneficiary's advance after an overturned review, cap $0.90
//   corner-pressure-> pressing team's advance on a 2+ corner lead in 10 min, cap $0.80
//   injury / free-kick -> no attributable directional signal; scan-only
const ADV_PROP = { Argentina: "argentina-advances", Switzerland: "switzerland-advances" };
const momentum = [];
function momentumEntry(agentId, ts, side, signal, cap) {
  const prop = ADV_PROP[side];
  const price = ksAt(KS_TICK[prop], ts, "mean");
  if (price == null || price > cap || price <= 0.02) return false;
  momentum.push({
    id: `mom-${momentum.length + 1}`,
    agentId,
    enteredAt: ts,
    minute: matchMinute(ts),
    side,
...
momentumEntry("goal-reaction", T.goal1, "Argentina", "Argentina score — momentum entry on the scorer's path", 0.9);
momentumEntry("goal-reaction", T.goal2, "Switzerland", "Switzerland equalize — momentum entry on the scorer's path", 0.9);
momentumEntry("goal-reaction", T.etGoal1, "Argentina", "Argentina lead in extra time — momentum entry", 0.9);
momentumEntry("goal-reaction", T.etGoal2, "Argentina", "Argentina seal it — momentum entry", 0.9);
momentumEntry("red-card", T.red, "Argentina", "Switzerland down to ten — back the eleven", 0.9);
momentumEntry("penalty-var", T.varEnd, "Argentina", "Review overturned against Switzerland — back the beneficiary", 0.9);
for (let ts = T.kickoff + 5 * 60_000; ts <= T.regEnd; ts += 60_000) {
  for (const side of ["Argentina", "Switzerland"]) {
    ...
    if (lead >= 2 && momentumEntry("corner-pressure", ts, side, `${side} +${lead} corners in the trailing 10 minutes`, 0.8)) {
      ts = T.regEnd + 1; // one taken pressure entry per match
      break;
    }
  }
}
momentum.sort((a, b) => a.enteredAt - b.enteredAt);
...
export const ARG_SUI_MOMENTUM_TRADES: readonly MomentumTrade[] = ${JSON.stringify(momentum, null, 2)} as const;
`;
...
console.log(`momentum entries: ${momentum.length}`);
for (const d of momentum) console.log(`  ${d.id} [${d.agentId}] ${d.side} @ $${(d.entryPriceMicros / USD).toFixed(2)} won=${d.won} — ${d.signal}`);
